...
class TimeStampUpdate(models.Model):
    '''
    Custom update of timestamps(created, updated) rather than relying on 
        auto_now and auto_update
    On save, update timestamps. 
        This approach is more reliable than auto_now or auto_now_add 
        (Also possible future deprecation of auto_now and auto_update)
...
class House(TimeStampUpdate):
    '''
    @Note : Houses can be identified by address(Street address, Unit, City, 
                State/Province, Zip/Post Code, Country)
            No need to have a separate model for furnace since every house 
                has a single furnace. 
            ...
            **Thermostat can regulate the temperature to keep it stable.
            >> PROS : if all rooms are at their ideal temperature, turn furnace 
                off to SAVE ENERGY.
            
            Example room temperatures: RT1 = 29deg, RT2 = 28deg, RT3 = 27deg, 
                RT4 = 26deg, RT5 = 25deg
            
            FURNACE STATUS: 
                OFF  : RT(s) = Normal or within range.
                HEAT : Turn on Furnace to Max(RT). Thermostats bring down 
                           temperatures in rooms with lower RT requirements.
                       >> PROS : SAVE ENERGY. Furnace will not be heated more 
                           than necessary. eg. Max(RT) = RT1 = 29deg
                FAN  : Run fan till Min(RT) is attained. Thermostats regulates 
                        temperature by regulating amount of air allowed in rooms 
                            with higher RT.
                       In some cases, vents could be shut to prevent temperature
                           cooling below RT in rooms with higher RT requirements.
                       >> PROS : SAVE ENERGY. Furnace will not blow too much 
                           air than necessary. eg. Min(RT) = RT5 = 25deg
    ...
    # street number and street name. eg. 23 Pelham St.
    ...
    # A furnace is in a room so it has a temperature too.
    furnace_temperature = models.DecimalField("Furnace Temperature", 
        max_digits=5, 
        decimal_places=2
    ) 
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id:
            ...
            rqset_max = Room.objects.filter(house_id=self.id).aggregate(Max('room_temperature'))
            self._max_temp = rqset_max.get('room_temperature__max')
            # Get the Min(room temp) for all rooms in this house instance.
            rqset_min = Room.objects.filter(house_id=self.id).aggregate(Min('room_temperature'))
            self._min_temp = rqset_min.get('room_temperature__min')
